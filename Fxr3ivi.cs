...
namespace FXRTestBed
{
    ...
    /// </summary>
    public class Fxr3ivi : SoulsFile<Fxr3ivi>
    {
        ...
        public class FFXActionCall
        {
            ...
            internal void WriteFields(BinaryWriterEx bw, int index, ref int fieldCount)
            {
                ...
                // {
                //     Console.WriteLine($"I found 0 fields!");
                //     bw.FillInt32(string.Format("ActionCallFieldsOffset[{0}]", index), 0);
                // }
                // else
                // {
                    // Console.WriteLine($"I found {Fields1.Count} field1s and {Fields2.Count} field2s");
    ...
    {
        public static Fxr3ivi XMLToFXR3(XDocument XML)
        {
            XmlSerializer test = new XmlSerializer(typeof(Fxr3ivi));
            XmlReader xmlReader = XML.CreateReader();

            return (Fxr3ivi)test.Deserialize(xmlReader);
        }

        public static XDocument FXR3ToXML(Fxr3ivi fxr)
        {
            XDocument XDoc = new XDocument();

            using (var xmlWriter = XDoc.CreateWriter())
            {
                var thing = new XmlSerializer(typeof(Fxr3ivi));
